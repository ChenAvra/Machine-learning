...
class OurGUI:
    def __init__(self, master):
        self.master = master
        master.title("K Means Clustring")
        ...
        master.button1.grid(column=2, row=4)
    ...
    def preProc(self):
        ...
        if self.filename is None:
            messagebox.showerror("K Mean Clustring", "You should enter a valid path file name before pre processing")
        else:
            try:
                self.df = preProcessing.preProcess(self.filename)

                if self.df is not None:
                    messagebox.showinfo("K Mean Clustring", "Preprocessing completed successfully!")
                    self.master.button1.config(state='active')

                    print(self.df)
            except:
                messagebox.showerror("K Mean Clustring", "error while  the pre processing")
    #model
    def model(self):
        try:
            if (int(self.e2.get())) > self.df.shape[0] or (int(self.e2.get())) < 3:
                messagebox.showerror("K Mean Clustring", "please enter a valid number of clusters")

            else:
                try:
                    if (int(self.e3.get())) > 50 or (int(self.e3.get())) < 1:
                        messagebox.showerror("K Mean Clustring", "please enter a valid number of runs")
                    else:
                        try:
                            ...
                            messagebox.showinfo("K Mean Clustring", "clustring completed successfully!")
                            ...
                            messagebox.showerror("K Mean Clustring", "error while the model")
